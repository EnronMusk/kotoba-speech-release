...
class TTS:
    def __init__(
        ...
        self._first_model_dir = snapshot_download(repo_id=model_name)
        if model_name != "kotoba-tech/kotoba-speech-v0.1":
            self._other_model_dir = snapshot_download(repo_id="metavoiceio/metavoice-1B-v0.1")
        else:
            self._other_model_dir = self._first_model_dir

        self.first_stage_adapter = FlattenedInterleavedEncodec2Codebook(end_of_audio_token=1024)
        self.output_dir = output_dir
        os.makedirs(self.output_dir, exist_ok=True)

        second_stage_ckpt_path = f"{self._other_model_dir}/second_stage.pt"
        ...
        self.llm_second_stage = Model(
            config_second_stage, TrainedBPETokeniser, EncodecDecoder, data_adapter_fn=self.data_adapter_second_stage.decode
        )
        self.enhancer = get_enhancer("df")
        print(str(Path(f"{self._other_model_dir}/first_stage.pt")))

        self.precision = {"float16": torch.float16, "bfloat16": torch.bfloat16}[self._dtype]
        self.model, self.tokenizer, self.smodel, self.model_size = build_model(
            precision=self.precision,
            checkpoint_path=Path(f"{self._other_model_dir}/first_stage.pt"),
            spk_emb_ckpt_path=Path(f"{self._other_model_dir}/speaker_encoder.pt"),
            device=self._device,
            compile=True,
            compile_prefill=True,
            first_model_path= first_model_path if (first_model_path is not None) and (os.path.exists(first_model_path)) else Path(f"{self._first_model_dir}/first_stage.pt"),
        )
    ...
    def synthesiseSequential(self, text: str, spk_ref_path: str, top_p=0.95, guidance_scale=0.0, temperature=1.0) -> str:
        """
        text: Text to speak
        spk_ref_path: Path to speaker reference file. Min. 30s of audio required. Supports both local paths & public URIs. Audio formats: wav, flac & mp3
        top_p: Top p for sampling applied to first-stage model. Range [0.9, 1.0] are good. This is a measure of speech stability - improves text following for a challenging speaker
        ...
        #Audio input for speech continuation.
        audio_encoder = EncodecDecoder(data_adapter_fn=self.data_adapter_second_stage, output_dir="", tokeniser_decode_fn=self.tokenizer)
        audio_toks = audio_encoder.get_tokens(audio_path=spk_ref_path)
